...
Sons=dict(
    ...
    Tiro=pygame.mixer.Sound(os.path.join(dirp,'Tiro.wav')),
    Kill=pygame.mixer.Sound(os.path.join(dirp,'Kill.wav')),
    Death=pygame.mixer.Sound(os.path.join(dirp,'Death.wav')),
    Pw_som=pygame.mixer.Sound(os.path.join(dirp,'Powercube.wav')),
    TP=pygame.mixer.Sound(os.path.join(dirp,'TP.wav')),
...
def pausar(pause,pontos,tela,maxpt,mute,*gps):
    '''
    pausa o jogo
    '''
    ...
    while True:
        ...
        tela.blit(fonte.render('Pausado!',False,(255,255,255)),(550,450))
        for i in gps:
            i.draw(tela)
        tela.blit(fonte.render('Pontos:%i'%pontos,False,(255,255,255)),(0,0))#Imprime a quantidade de pontos do jogador
        tela.blit(fonte.render('Maxpt:%i'%maxpt,False,(255,255,255)),(540,0))#Imprime a quantidade máxima de pontos do jogador
        if mute:
...
def colisão(player,va,tela,te,mute,morreu,pontos,*gps):
    ...
    for i in gps[5]:#Colisão P>E
        ...
        if col_pe:
            ...
            col_pa[0].kill()
            Sons['Kill'].play() if not(mute) else None
            pontos+=1
    ...
    for i in gps[4]:#colisão A>E
        ...
        if col_ae:
            ...
            player.kill()
            Sons['Death'].play() if not(mute) else None
            morreu=True
    ...
    if pygame.sprite.spritecollide(player,gps[3],True,pygame.sprite.collide_mask):
        ...
        Sons['Pw_som'].play() if not(mute) else None

    return morreu,pontos,te

def Atirar_p(coord_tiro,te,player,mute,gp_tp):
    '''
    ...
    if len(gp_tp)<3:
        ...
        if not(te):
            Sons['Tiro'].play() if not(mute) else None

        if te:
            tiro.pws=True
            te=False
            player.image=player.img[0]
            Sons['TP'].play() if not(mute) else None
